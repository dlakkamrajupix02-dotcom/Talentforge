...
import { getImportedJds, uploadImportPdf, deleteImportedJd, updateImportedJd, bulkConvertImportedJds } from "../../services/bulkImportService";
import { PDF_IMPORT_STATUS } from "../../utils/importedJd";
...
const mergePdfFiles = (existing, incoming) => {
...
const collectPdfFiles = (fileList) => {
...
export default function BulkImport() {
  ...
  const handleConfirmWorkflow = async (workflowId) => {
    ...
    try {
      const convertedJds = await bulkConvertImportedJds({ jd_ids: [selectedTemplate.id] });
  ...
  const loadImportedJds = async () => {
    setIsLoading(true);
    try {
      const data = await getImportedJds();
      setImportedTemplates(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error("Failed to fetch imported JDs:", error);
  ...
  const handleEditInDrawer = () => {
    ...
    setEditData({
      ...
      status: selectedTemplate.status || PDF_IMPORT_STATUS,
  ...
  const saveDrawerEdit = async () => {
    ...
    try {
      ...
      await updateImportedJd(selectedTemplate.id, {
  ...
  const handleDrop = useCallback((e) => {
    ...
    e.stopPropagation();
    setDragActive(false);
    const dropped = collectPdfFiles(e.dataTransfer.files);
    if (dropped.length === 0) {
      toast.error("Please upload valid PDF files (.pdf)");
      return;
    }
    if (dropped.length < e.dataTransfer.files.length) {
      toast.error("Some files were skipped — only PDF documents are supported");
    }
    setFiles((prev) => mergePdfFiles(prev, dropped));
  }, []);

  const handleFileSelect = (e) => {
    const selected = collectPdfFiles(e.target.files);
    if (selected.length === 0) {
      toast.error("Please select valid PDF files (.pdf)");
      e.target.value = "";
      return;
    }
  ...
  const handleImport = async () => {
    ...
    setIsUploading(true);
    try {
      const result = await uploadImportPdf(files);
      const importedCount = Array.isArray(result) ? result.length : files.length;
      toast.success(
        importedCount === 1
          ? "1 PDF imported successfully!"
          : `${importedCount} PDFs imported successfully!`
      );
      setFiles([]);
      ...
      toast.error(error.message || "Failed to import PDF file(s)");
    } finally {
        ...
        <div className="flex-1 max-w-xl z-10 text-center lg:text-left relative">
          ...
          <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 mt-6">
            ...
            <span className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-bold rounded-lg flex items-center gap-2">
              <Zap className="w-4 h-4" /> Real-time PDF Parser
            </span>
          </div>
        </div>
            ...
            {files.length === 0 ? (
              ...
              <div className="w-full flex flex-col items-center">
                ...
                >
                  {isUploading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" /> Importing {files.length} PDF{files.length !== 1 ? "s" : ""}...
                    </>
                  ) : (
                    <>
          ...
          {isLoading ? (
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  setEditData({
                                    ...
                                    status: template.status || PDF_IMPORT_STATUS,
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-xs font-semibold text-slate-400 max-w-sm">
                Upload PDF files above to populate imported records into the pipeline.
              </p>
            </div>
          )}
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-white/[0.02] rounded-2xl border border-slate-100 dark:border-white/5 flex flex-col justify-center">
                      ...
                      {isDrawerEditing ? (
                        ...
                        >
                          <option value={PDF_IMPORT_STATUS}>Imported</option>
